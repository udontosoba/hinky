 ...
 *・ロボットの状態は「初期状態(s_1)」「停止状態(s_2)」「待機状態(s_3)」「走行状態(s_4)」「設定状態(s_5)」の5状態．
 *・各状態の遷移関係は次の通り．
 *  プログラム開始 -> s_1
 *  s_1 -> s_2(キャリブレーション終了時)
 *  s_2 -> s_3(コース検知時), s_2 -> s_5(設定状態遷移行動検>知時)
 *  s_3 -> s_2(コース不検知時), s_3 -> s_5(設定状態遷移行動>検知時), s_3 -> s_4(走行状態遷移行動検知時)
 *  s_4 -> s_2(走行終了(ボールを手放す行動をしたとき)または>走行終了行動検知時)
 *  s_5 -> s_2(設定終了行動検知時)
 *  ・s_1
 *  各センサのpre(1周期前の値)とcur(現在の値)を格納し終える>まで待機する．
 *  黒と白と赤のキャリブレーションを行い，閾値の設定をする．
 *  ディスプレイ表示は「put on a black line and press hoge」「put on a red line and press hoge」「put on a white line and press hoge」など
 *  ・s_2
 *  ディスプレイの表示は「put on the course」と「time = 00:00.0」(maxは4分)と現在ロボットがいるコース
 *  ・s_3
 *  ディスプレイの表示は「put start button」と「time = 00:00.0」なお，コース手前のタイムから再計測し表示する．
 *  ・s_4
 *  走行終了(ボールを手放す行動をしたとき)または走行終了行動検知時(ボタン押下を想定)までモーターを回転させてコースを走行する．
 *  コースの認識をして走行戦略を変える．各コースの走行にかかったタイムは保持しておく．コースの手前からリトライするときにそのタイムから再計測し表示する．
 *  ・s_5
 *  PID値，ロボットが走行しているコースの設定ができる．また，現在の電圧を確認できる．
 *
 *  時間周期割り込みは，サンプリング時間(10ms = 0.01s)ごとに行われ，外部変数に格納される．開発者は外部変数が定義されているcファイルにアクセスする．(アクセスのための関数は用意する．)
 *  取得したタイムを用いて(if文)ディスプレイの更新や，モーター電圧の変更や，センサ値の取得を行う．
 *
 *  なお，黒は赤の上である．
 *
 * 00_ : 設定関係ファイル
...
#include "z_20_3_standby_state.h"
#include "z_20_4_run_state.h"
#include "z_20_5_set_state.h"
  ...
  while(1){
    switch (state) {
      ...
      case stop_state:
        ...
        break;
      case standby_state:
        state = func_z_20_3_standby_state_main();
        break;
      case run_state:
        state = func_z_20_4_run_state_main();
        break;
      case set_state:
        state = func_z_20_5_set_state_main();
        break;
      default:
        state = func_z_20_1_init_state_main();
		    break;
